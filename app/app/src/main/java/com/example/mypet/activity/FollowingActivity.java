...
import com.example.mypet.adapter.FollowingAdapter;
...
public class FollowingActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        followerService.findAllByFollower(idPet, token).enqueue(new Callback<ArrayList<Follower>>() {
            @Override
            public void onResponse(Call<ArrayList<Follower>> call, retrofit2.Response<ArrayList<Follower>> response) {
                if (response.isSuccessful()) {
                    ...
                    rvFollower.setLayoutManager(new LinearLayoutManager(FollowingActivity.this));
                    rvFollower.setAdapter(new FollowingAdapter(followerArrayList, FollowingActivity.this));

                }else{
                    Toast.makeText(FollowingActivity.this, "Sorry we had problems!", Toast.LENGTH_SHORT).show();
                }
            }

            @Override
            public void onFailure(Call<ArrayList<Follower>> call, Throwable t) {
                Toast.makeText(FollowingActivity.this, "   ", Toast.LENGTH_SHORT).show();
